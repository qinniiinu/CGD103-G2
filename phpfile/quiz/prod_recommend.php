...
if($style_id === 101){
    ...
    $sql = "select * from product where style_type LIKE ('%文青風%') limit 3";
        $book = $pdo->query($sql);
        $books = $book->fetchALL(PDO::FETCH_ASSOC);
        $data = [];
        foreach ($books as $i => $page) {
    ...
    $sql = "select * from product where style_type LIKE ('%時尚風%') limit 3";
        $book = $pdo->query($sql);
        $books = $book->fetchALL(PDO::FETCH_ASSOC);
        $data = [];
        foreach ($books as $i => $page) {
    ...
    $sql = "select * from product where style_type LIKE ('%極簡風%') limit 3";
        $book = $pdo->query($sql);
        $books = $book->fetchALL(PDO::FETCH_ASSOC);
        $data = [];
        foreach ($books as $i => $page) {
