      ...
      <div class="member_side_menu">
        <div class="member">
          ...
          <div class="name_level">
            ...
            <span v-if="Iflevel" class="level"
              >{{ levelInfo.level_name }} Account</span
            >
            <router-link
              v-if="!level_top"
              to="/MyPage/memSubscription"
              class="level_up"
              >立即升級</router-link
            >
          </div>
  ...
  methods: {
    ...
    isActive(option) {
      ...
      if (option.url === this.$route.path) {
        ...
        option.url === "/MyPage/OrderHistory" &&
        this.$route.path.startsWith("/MyPage/OrderHistory")
      ) {
        return true;
      }
      return false;
    ...
    getResourse() {
      this.axios
        .get(`${BASE_URL}/memberInfo.php`,{ credentials: "include" })
        .then((res) => {
          ...
          if (res.data.level_id) {//有訂閱，顯示帳號等級
            ...
            // 對照訂閱等級 vip_level
            this.axios
              .post(`${BASE_URL}/member_level.php`, formData, {
                credentials: "include",
              })
